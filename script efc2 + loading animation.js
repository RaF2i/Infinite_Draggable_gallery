...
// START IMMEDIATELY - No waiting for anything
...
function createInitialItems() {
  ...
  initialVisibleItems.forEach(itemId => {
    ...
    item.style.transform = 'translateY(64px) scale(1.5)';
...
// Start loading animation for only visible items
function startLoadingAnimation() {
  //  time from 10ms to 5ms and max timeout to 300ms 
  if (typeof gsap === 'undefined') {
    const startTime = Date.now();
    const checkGsap = () => {
      if (typeof gsap !== 'undefined' || Date.now() - startTime > 100) {
        performLoadingAnimation();
      } else {
        setTimeout(checkGsap, 5);
      }
    };
    checkGsap();
    return;
  ...
  // If GSAP is available, ensure animation does not start before 300ms
  setTimeout(performLoadingAnimation, 300);
}
...
function performLoadingAnimation() {
  ...
  // Animate only the initially visible items with increased duration
  if (typeof gsap !== 'undefined') {
    gsap.to(initialItems, {
      opacity: 1,
      y: 0,
      scale: 1,
      duration: 0.8, // Increased from 0.6 to 0.8 (200ms increase)
      stagger: 0.02,
...
function updateVisibleItems() {
  ...
  for (let row = startRow; row <= endRow; row++) {
    for (let col = startCol; col <= endCol; col++) {
      ...
      // New items after initial load - animate in with increased duration
      if (initialAnimationDone && window.gsap) {
        gsap.fromTo(item,
          { opacity: 0, y: 64, scale: 1.5 },
          {
            opacity: 1,
            y: 0,
            scale: 1,
...
startLoadingAnimation(); // Start animation as soon as possible
