...
function initNavAnimation() {
  ...
  // Check for SplitType availability
  ...
  // If SplitType is available immediately
  performNavAnimation(nav);
}

function performNavAnimation(nav) {
  // Create SplitType instance for characters
  const navSplit = new SplitType(nav, { types: "chars" });
  
  // Set initial state - characters at bottom
  if (window.gsap) {
    gsap.set(navSplit.chars, { y: "100%" });
    ...
    gsap.to(navSplit.chars, {
      ...
      stagger: 0.05,
      ease: "power3.out",
      delay: 0.1 // Small delay after page load
    });
  }
}
