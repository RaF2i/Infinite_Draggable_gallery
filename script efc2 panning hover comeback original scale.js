...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const panStrength = 0.4; // How much the mouse affects panning (0-1)
  const panRange = 250; // Maximum pan distance in pixels

  // Blur effect variables (only for other items during expansion/collapse)
  let blurAmount = 0;
  let targetBlur = 0;
  ...
  function updateBlur() {
    const ease = 0.1;
    blurAmount += (targetBlur - blurAmount) * ease;
    
    // Only apply blur to the canvas when it's supposed to be blurred (i.e., other items are blurred)
    if (!isExpanded) { // Only update canvas blur when not expanded
        if (Math.abs(targetBlur - blurAmount) > 0.01) {
            canvas.style.filter = `blur(${blurAmount}px)`;
            requestAnimationFrame(updateBlur);
        } else {
            blurAmount = targetBlur;
            canvas.style.filter = targetBlur > 0 ? `blur(${blurAmount}px)` : 'none';
        }
    }
  }
  ...
  function handleMousePan(e) {
    ...
    // Calculate mouse position as decimal (0 to 1)
    const xDecimal = mouseX / window.innerWidth;
    const yDecimal = mouseY / window.innerHeight;

    // Calculate pan offset (centered around 0.5)
    ...
    // Start panning animation if not already active
    if (!panActive) {
      panActive = true;
      startPanAnimation();
    }
  ...
  function startPanAnimation() {
    ...
    panAnimation = gsap.to({}, {
      ...
      onUpdate: function() {
        ...
        // Smooth interpolation for pan movement
        const panEase = 0.08;
  ...
  function expandItem(item) {
    ...
    // Reset pan values when expanding
    panTargetX = 0;
    panTargetY = 0;
    panCurrentX = 0;
    panCurrentY = 0;
    ...
    // Store the item's original bounds (unscaled position and size)
    ...
    originalPosition = {
      ...
      rect: rect,
      imgSrc: targetImg,
      originalScale: hoveredItem === item ? 1.2 : 1, // Store the scale it was at
      // Store the original item position in the canvas (unscaled)
    ...
    // Apply blur to other items and fade them out
    document.querySelectorAll(".item").forEach((el) => {
      if (el !== activeItem) {
        gsap.to(el, {
          opacity: 0,
          filter: "blur(5px)", // Add blur to other items
          duration: 0.3,
          ease: "power2.out",
        });
      }
    ...
    // Calculate 85vh height and maintain aspect ratio
    ...
    // Start from scale 1.2 if item was hovered
    ...
    gsap.fromTo(
      expandedItem,
      {
        ...
        filter: "blur(0px)" // The selected image does not have blur
      },
      {
        width: targetWidth,
        height: targetHeight,
  ...
  function closeExpandedItem() {
    ...
    // Start title animation out immediately
    animateTitleOut(() => {
      // This callback ensures title is fully cleared before proceeding
    });
    ...
    gsap.delayedCall(0.3, () => {
      ...
      // Calculate the original item's position in viewport coordinates
      // This accounts for the current canvas transform
      ...
      // Calculate the actual screen position of the original item (unscaled)
      ...
      document.querySelectorAll(".item").forEach((el) => {
        if (el.id !== activeItemId) {
          gsap.to(el, {
            opacity: 1,
            filter: "blur(0px)", // Remove blur from other items
            duration: 0.3,
            ease: "power2.out",
          });
        }
      ...
      gsap.to(expandedItem, {
        width: itemWidth, // Always return to original item's unscaled width
        height: itemHeight, // Always return to original item's unscaled height
        x: screenX - window.innerWidth / 2, // Use calculated screen position
        y: screenY - window.innerHeight / 2, // Use calculated screen position
        duration: 1,
        ease: "hop",
        filter: "blur(0px)", // The returning selected image does not have blur
        onComplete: () => {
          ...
          if (originalItem) {
            ...
            // Set scale to 1 immediately (no animation needed as it should be at original scale)
            gsap.set(originalItem, { scale: 1 });
          }

          // Force clear the title if it's still there
          if (titleSplit) {
            titleSplit.revert();
          }
          projectTitleElem.textContent = '';
          ...
          dragVelocityY = 0;
          hoveredItem = null;
          isAnimatingTitle = false; // Reset animation state
        },
      });
    });
  }
  ...
  function animate() {
    if (canDrag) {
      ...
      // Add mouse panning effect to the transform
      const finalX = currentX + panCurrentX;
      const finalY = currentY + panCurrentY;
